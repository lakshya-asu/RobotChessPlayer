...
mapfile -t ROS_PYTHONPATHS < <(build_ros_python_paths)
...
if [ -n "${ISAAC_BRIDGE_PYTHONPATH}" ]; then
  PYTHONPATH_SEGMENTS+=(${ISAAC_BRIDGE_PYTHONPATH// / })
fi
if [ "${#ROS_PYTHONPATHS[@]}" -gt 0 ]; then
  PYTHONPATH_SEGMENTS+=("${ROS_PYTHONPATHS[@]}")
fi
PYTHONPATH_SEGMENTS+=("${REPO_ROOT}")
export PYTHONPATH="$(printf '%s\n' "${PYTHONPATH_SEGMENTS[@]}" | awk '!seen[$0]++' | paste -sd: -)"
