...
class ChessManagerNode(Node):
    ...
    def _execute_move(self, goal_handle) -> ExecuteChessMove.Result:
        ...
        try:
            ...
            self._publish_status("planning", move)
            ...
            execution_result = self._forward_to_executor(move)
    ...
    def _forward_to_executor(self, move: str) -> ExecuteChessMove.Result:
        if not self._white_executor.wait_for_server(timeout_sec=5.0):
            raise RuntimeError("White robot executor is unavailable.")
        goal = ExecuteChessMove.Goal()
        goal.uci_move = move
        send_future = self._white_executor.send_goal_async(goal)
        ...
        if not send_future.done():
            raise RuntimeError("Timed out dispatching move to white executor.")
        goal_handle = send_future.result()
        if goal_handle is None or not goal_handle.accepted:
            raise RuntimeError("White robot executor rejected the move goal.")
        result_future = goal_handle.get_result_async()
        while not result_future.done() and time.monotonic() < deadline:
            time.sleep(0.05)
        if not result_future.done():
            raise RuntimeError("Timed out waiting for white executor result.")
