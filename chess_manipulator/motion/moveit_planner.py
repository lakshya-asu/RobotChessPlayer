...
class MoveItTrajectoryPlanner:
    ...
    def __init__(
        ...
        self._ik_client = node.create_client(
            GetPositionIK,
            self._config.compute_ik_service,
            callback_group=self._callback_group,
        )
        self._plan_client = node.create_client(
            GetMotionPlan,
            self._config.motion_plan_service,
            callback_group=self._callback_group,
        )
        self._planning_scene_client = node.create_client(
            ApplyPlanningScene,
            self._config.apply_planning_scene_service,
            callback_group=self._callback_group,
        )
    ...
    def set_calibration(self, calibration: BoardCalibration) -> None:
        self._calibration = calibration
    ...
    def plan_move_from_positions(
        ...
        for name, pose in self._stage_specs(description, capture_slot):
            ...
            stage_end_times_sec.append(self._duration_sec(combined.points[-1].time_from_start))
            current_positions = list(target_positions)

        return MovePlan(
            move=description.uci,
            stages=stages,
            stage_end_times_sec=stage_end_times_sec,
            trajectory=combined,
        )

    def _wait_for_services(self) -> None:
        if not self._ik_client.wait_for_service(timeout_sec=5.0):
            raise RuntimeError(f"{self._config.compute_ik_service} is not available.")
        if not self._plan_client.wait_for_service(timeout_sec=5.0):
            raise RuntimeError(f"{self._config.motion_plan_service} is not available.")
        if not self._planning_scene_client.wait_for_service(timeout_sec=5.0):
            raise RuntimeError(f"{self._config.apply_planning_scene_service} is not available.")
        self._configure_allowed_collisions_once()

    def _current_joint_positions(self) -> List[float]:
        if self._joint_state is None:
            raise RuntimeError(f"No {self._config.joint_state_topic} received for MoveIt planning.")
    ...
    def _stage_specs(
        ...
        if description.is_capture:
            captured_hover = self._calibration.square_to_position(
                description.target,
                self._hover_height_m,
                robot_side=self._robot_side,
            )
            captured_pick = self._calibration.square_to_position(
                description.target,
                self._pickup_height_m,
                robot_side=self._robot_side,
            )
        ...
        source_hover = self._calibration.square_to_position(
            description.source,
            self._hover_height_m,
            robot_side=self._robot_side,
        )
        source_pick = self._calibration.square_to_position(
            description.source,
            self._pickup_height_m,
            robot_side=self._robot_side,
        )
        target_hover = self._calibration.square_to_position(
            description.target,
            self._hover_height_m,
            robot_side=self._robot_side,
        )
        target_pick = self._calibration.square_to_position(
            description.target,
            self._pickup_height_m,
            robot_side=self._robot_side,
        )
        ...
        if description.is_castling:
            ...
            stages.extend(
                [
                    (f"castle_rook_{name}", pose)
                    for name, pose in self._stage_specs(rook_desc, capture_slot)
                ]
            )
