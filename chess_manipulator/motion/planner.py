...
class MovePlan:
    ...
    stage_end_times_sec: List[float] = field(default_factory=list)
    ...
    def plan_move(self, description: MoveDescription, capture_slot: int = 0) -> MovePlan:
        ...
        if description.is_castling:
            ...
            stages.extend(
                [
                    PlannedStage(
                        name=f"castle_rook_{stage.name}",
                        pose=stage.pose,
                        joints=stage.joints,
                    )
                    for stage in rook_plan.stages
                ]
            )

        stage_end_times_sec = [self._stage_time_sec * index for index in range(1, len(stages) + 1)]
        return MovePlan(
            move=description.uci,
            stages=stages,
            stage_end_times_sec=stage_end_times_sec,
            captured_dropoff=captured_dropoff,
        )
